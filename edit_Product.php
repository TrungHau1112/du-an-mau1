...
if (!isset($_GET['id'])) {
    header("Location: product.php"); // Redirect if no product ID is provided
    exit;
}
...
// Define valid sizes
$valid_sizes = ['S', 'M', 'L', 'XL', 'XXL']; // Add any sizes you want to allow
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Get other product details from form
    $data['name'] = $_POST['name'];
    $data['description'] = $_POST['description'];
    $data['price'] = $_POST['price'];
    $data['category_id'] = $_POST['category_id'];
    $data['quantity'] = $_POST['quantity']; // New field for quantity
    $data['size'] = $_POST['size']; // New field for size

    // Validate the size (check if the size is in the list of valid sizes)
    if (!in_array($data['size'], $valid_sizes)) {
        echo "Invalid size selected.";
        exit; // Stop the script if invalid size is selected
    }

    // Update product details in the database
    $db->update('products', $data, 'id = :id', ['id' => $product_id]);

    // Redirect back to the products page after editing
